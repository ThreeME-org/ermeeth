...
variables_selection <-   c(variables_like(data_full,"^F_L_S[A-Z0-9]{3}$",FALSE),"F_L",
                           ...
                           variables_like(data_full,"^EMS_CI_CO2_C[A-Z0-9]{3}$",FALSE),"EMS_CI_CO2",
                           variables_like(data_full,"^EMS_CH_CO2_C[A-Z0-9]{3}$",FALSE),"EMS_CH_CO2",
                           "GDP","PGDP","EMS_CO2",
...
data_ratio <- rbind(dplyr::filter(data_full,variable %in% variables_selection),
  ...
  list_S_variable <- c("F_L","F_K","F_MAT","F_E","C_L","C_K","C_MAT","C_E","I","Y","PY") %>%
    ...
    reduce(c)
  list_C_variable <- c("MGS","CI","CH","G","I","X","M","F_E","EMS_CI_CO2","EMS_CH_CO2") %>%
...
calcul_commodity_functions <- function(commodity, data){
 ...
 res <- mutate(data,
               ...
               !!paste0("PHI_EMS_CI_",commodity) := get(paste0("EMS_CI_CO2_",commodity))/EMS_CI_CO2,
               !!paste0("PHI_EMS_CH_",commodity) := get(paste0("EMS_CH_CO2_",commodity))/EMS_CH_CO2
               ) %>%
      select(year,
             ...
             all_of(paste0("PHI_EMS_CI_",commodity)),
...
PHI_EMS_GDP = EMS_CO2 / GDP,
PHI_EMS_CI_GDP = EMS_CI_CO2 / GDP,
PHI_EMS_CH_GDP = EMS_CH_CO2 / GDP)
