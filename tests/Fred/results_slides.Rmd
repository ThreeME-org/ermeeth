...
contrib_comp <- c( "CH","BAL_TRADE", "I", "G","DS")
contrib_comp_label <- c( "Households consumption","Trade balance", "Investment", "Public spending","Stock variation")
...
A <- contrib.plot(data_plot, series = contrib_comp,label_series = contrib_comp_label,
             startyear = 2020, titleplot = "Contributions to change in GDP (in difference from baseline)", line_tot = TRUE)


contrib_comp <- c( "X", "M")
contrib_comp_label <- c("Exports", "Imports")

data_plot <-  contrib(data_full,"BAL_TRADE", contrib_comp, scenar = c("baseline",scenario_name), indicator = "rel.diff", neg.value = "M")
...
A <- curve_sc_plot(data_sector,"EMS_CI_CO2", group_type = "commodity", growth.rate = FALSE,diff = TRUE,abs.diff = FALSE, title = "GHG emissions per source (in difference from baseline)")
