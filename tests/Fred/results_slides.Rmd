...
```{r GHG emissions per source, eval=FALSE, fig.height=6, fig.width=14, include=FALSE}

variables_selection = c("EMS_CI", "EMS_CH")
new_indicators <- data_sector %>% 
  # 1. Put in Wide format
  wide_data(variables = variables_selection , out_format = "list") %>%  ## 1. passe en wide
  # 2. Calculate indicators
  map(~.x %>%          
  ...
  long_data() %>%
  rbind(data_sector) %>% ## 4. on recolle à la base
  unique() ## 5. on enleve les doublons



A <- curve_sc_plot(new_indicators,"EMS_CI_CH", group_type = "C", growth.rate = FALSE,diff = TRUE,abs.diff = FALSE, title = "GHG emissions per source (in difference from baseline)")

data_plot <- contrib.sub(data_sector,var1 =  "EMS",
...
```{r CO2 emissions per source, fig.height = 6, fig.width=14}
