...
```{r PAUL: contrib BUG WAGES, eval=FALSE, fig.height=6, fig.width=12, include=FALSE}


variables_selection = c("GOS", "WAGES", "RSC", "NTAXS") 

#  En faisant la transformation : VA = GOS + RSC + NTAXS, Ca marche 
new_indicators <- data_full %>% 
  # 1. Put in Wide format
  wide_data(variables = variables_selection , out_format = "list") %>%  ## 1. passe en wide
  # 2. Calculate indicators
...
contrib_comp <- c("GOS", "RSC", "NTAXS")
contrib_comp_label <- c("GOS","Employers social security contribution", "Net taxes on the production")

data_plot <-  contrib(new_indicators,"VA", contrib_comp, scenar = c("baseline",scenario_name))
...
# En remplaçant GOS par WAGES (ques est dans la base), ça plante:
...
contrib_comp <- c("WAGES", "RSC", "NTAXS")
contrib_comp_label <- c("Wages","Employers social security contribution", "Net taxes on the production")
