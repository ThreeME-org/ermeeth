...
# Exported GHG emissions: decomposition per source

```{r ExporTed GHG emissions per source, fig.height=6, fig.width=14}


A <- curve_sc_plot(data_com_sec,"EMS_X", startyear = startyear, endyear = endyear, group_type = "C", growth.rate = FALSE,diff = TRUE,abs.diff = FALSE, title = "GHG emissions per source (in difference from baseline)")

data_plot <- contrib.sub(data_com_sec,var1 =  "EMS_X",
group_type = "C", scenar = c("baseline", scenario_name))
