...
threeme_wide_df <- data_agg %>% wide_data(variables = c(variables_like(.,"^Y_",FALSE),"GDP") , 
...
threeme_wide_list <- data_agg %>% wide_data(out_format = "list") 
...
data_mod <- data_agg %>% 
  wide_data(variables = c("M","GDP") , out_format = "list") %>%  ## 1. passe en wide
