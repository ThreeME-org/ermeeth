...
```{r test data}
secteurs <- data %>% filter(grepl("_C\\w{3}$",variable)) %>% select(commodity,variable) %>% mutate(variable = str_extract(variable, "C\\w{3}$")) %>% unique() %>% arrange(variable)
secteurs
```
...
data_plot <-  contrib(data,"GDP", GDP_comp, scenar = c("baseline",scenario_name), indicator = "gr.diff")
...
# For a selected scenario, contributions to yearly growth rate (in pp)
data_plot <-  contrib(data,"GDP", GDP_comp, scenar = c( "oilprice_fra","baseline"), indicator = "gr.diff")
