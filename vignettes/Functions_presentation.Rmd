...
dates <- c(1990 , 1995 , 2000 , 2010, 2015 )
values <- c(107  , 120  , 84   , 42  , 78)

interpolation_series(date_vector = dates,
                     value_vector = values,
                     first.date = 1990 , last.date = 2020)

## Pour voir graphiquement ce que donne l'interpolation
interpolation_series( date_vector =  dates,
                     value_vector = values,
                     first.date = 1990 , last.date = 2012, full_results = TRUE)
...
my_data <- data.frame(year = c(first_date_x:last_date_x), gdp = 100) %>%
  mutate(gdp = 100 * 1.013 ^( year - first_date_x ),
         target = interpolation_series(date_vector = c(2005,2015,2025,2035,2045,2050),
