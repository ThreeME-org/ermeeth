...
data_test_subsectors <- all_subsectors %>%
  ...
  mutate( !!scenario_name := 2*baseline , commodity = NA, 
          sector = str_replace_all(tolower(str_extract(variable,"S\\w{3}$")), set_names(names_sectors$name,names_sectors$code) ) ) 
...
data_test_sectors <- all_sectors %>%
  ...
  tidyr::pivot_longer(cols = !year) %>% rename(baseline = value , variable = name) %>%
  mutate( !!scenario_name := 2*baseline , commodity = NA, 
          sector = str_replace_all((str_extract(variable,"S\\w{3}$")), set_names(names_sectors$name,names_sectors$code) ) ) 
